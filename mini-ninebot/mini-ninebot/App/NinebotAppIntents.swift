...
struct NinebotOpenRecordingIntent: AppIntent {
    static var title: LocalizedStringResource = "打开九号记录"
    static var description = IntentDescription("打开 NineBot+ 的记录页面，准备开始记录 GPS 速度、加速 G 值和轨迹。")
    static var openAppWhenRun = true

    func perform() async throws -> some IntentResult & ProvidesDialog {
        await MainActor.run {
            NinebotSharedStore().savePendingAppRoute(.recording)
        }
        return .result(dialog: "已打开记录页面")
    }
}

struct NinebotOpenTripsIntent: AppIntent {
    static var title: LocalizedStringResource = "打开九号行程"
    static var description = IntentDescription("打开 NineBot+ 的行程页面查看接口行程、趋势和详情。")
    static var openAppWhenRun = true

    func perform() async throws -> some IntentResult & ProvidesDialog {
        await MainActor.run {
            NinebotSharedStore().savePendingAppRoute(.trips)
        }
        return .result(dialog: "已打开行程页面")
    }
}
...
struct NinebotAppShortcutsProvider: AppShortcutsProvider {
    ...
    static var appShortcuts: [AppShortcut] {
        ...
        AppShortcut(
            intent: NinebotOpenRecordingIntent(),
            phrases: [
                "\(.applicationName)开始记录",
                "用\(.applicationName)开始记录",
                "在\(.applicationName)开始骑行记录",
                "\(.applicationName)打开记录",
                "\(.applicationName)记录骑行",
        ...
        AppShortcut(
            intent: NinebotOpenTripsIntent(),
            phrases: [
                "\(.applicationName)打开行程",
                "用\(.applicationName)查看行程",
                "在\(.applicationName)查看行程",
                "\(.applicationName)行程记录",
                "\(.applicationName)查看趋势"
            ],
            shortTitle: "行程",
...
private enum NinebotShortcutRunner {
    ...
    private static func refreshedDashboardIfPossible(store: NinebotSharedStore) async throws -> NinebotDashboard {
        ...
        let cached = store.loadDashboard()
        let dashboard = try await client.fetchDashboard(selectedSN: cached?.selectedSN)
        return saveDashboardAndSync(dashboard, store: store)
    }

    private static func saveDashboardAndSync(_ dashboard: NinebotDashboard, store: NinebotSharedStore) -> NinebotDashboard {
