...
struct NinebotDashboardView: View {
                    ...
                    VStack(alignment: .leading, spacing: 18) {
                        if let primary = model.dashboard.primaryVehicle {
                            ...
                            VehicleControlHero(
                                ...
                                isCapturePrivacyEnabled: isCapturePrivacyEnabled,
                                isCapturePrivacyObscured: isCapturePrivacyObscured,
    ...
    private var isCapturePrivacyEnabled: Bool {
        model.capturePrivacyProtectionEnabled
    }
    ...
    private static var currentScreenIsCaptured: Bool {
...
private struct CapturePrivacyProtectionModifier: ViewModifier {
    var isEnabled: Bool
    var isObscured: Bool
    ...
    func body(content: Content) -> some View {
        if isEnabled {
            ScreenCaptureProtectedView(
                ...
                CapturePrivacyPlaceholder(cornerRadius: cornerRadius, message: message)
            }
            .animation(.easeInOut(duration: 0.18), value: isObscured)
        } else {
            content
        }
    }
...
private struct CapturePrivacyPlaceholder: View {
    var cornerRadius: CGFloat
    var message: String?
    ...
    var body: some View {
        if let message {
            RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                ...
                .overlay {
                    Label(message, systemImage: "eye.slash.fill")
...
private extension View {
    func capturePrivacyProtected(
        isEnabled: Bool,
        isObscured: Bool,
        cornerRadius: CGFloat,
        message: String? = nil
    ) -> some View {
        modifier(CapturePrivacyProtectionModifier(
            isEnabled: isEnabled,
            isObscured: isObscured,
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                BatteryHistoryTrendCard(points: points)
                BatteryChemistrySettingsCard(model: model, snapshot: snapshot)
...
private struct BatteryChemistrySettingsCard: View {
    @ObservedObject var model: NinebotViewModel
    var snapshot: NinebotVehicleSnapshot
    @State private var selection: NinebotBatteryChemistry
    @State private var isExpanded: Bool
    @State private var nominalVoltageInput: String
    @State private var capacityWhInput: String

    init(model: NinebotViewModel, snapshot: NinebotVehicleSnapshot) {
        self.model = model
        self.snapshot = snapshot
        let chemistry = snapshot.state.serverPrediction?.batteryChemistry
        let configured = chemistry?.configured ?? .auto
        _selection = State(initialValue: configured == .auto ? .leadAcid : configured)
        _isExpanded = State(initialValue: false)
        _nominalVoltageInput = State(initialValue: Self.inputText(chemistry?.nominalVoltage))
        _capacityWhInput = State(initialValue: Self.inputText(chemistry?.capacityWh))
    }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            if isExpanded {
                Picker("电池类型", selection: $selection) {
                    ForEach(NinebotBatteryChemistry.allCases) { chemistry in
                        Text(chemistry.title).tag(chemistry)
                    }
                }
                ...
                HStack(spacing: 10) {
                    BatterySpecificationInput(
                        title: "标称电压",
                        placeholder: "例如 72",
                        unit: "V",
                        text: $nominalVoltageInput
                    )
                    BatterySpecificationInput(
                        title: "额定容量",
                        placeholder: "例如 1440",
                        unit: "Wh",
                        text: $capacityWhInput
                    )
                }

                Text("容量按 Wh 填写，用于行程能耗和充电时间预测")
                    .font(.caption)
                    .foregroundStyle(Color.teslaSecondaryText)

                Button {
                    Task {
                        await model.updateBatteryChemistry(
                ...
                .tint(Color.teslaGreen)
                .disabled(model.isLoading || !hasValidSpecification)
            }
        }
        .padding(16)
        ...
        .onChange(of: chemistry) { _, value in
            if let value {
                selection = value.configured
                nominalVoltageInput = Self.inputText(value.nominalVoltage)
                capacityWhInput = Self.inputText(value.capacityWh)
                withAnimation(.easeInOut(duration: 0.2)) {
                    isExpanded = false
                }
            }
    ...
    private var effectiveStatusText: String {
        guard let chemistry = snapshot.state.serverPrediction?.batteryChemistry else {
            return "本车预设为铅酸，保存后用于两个预测模型"
        }
        if chemistry.effective == "unknown", selection != .auto {
            return "保存后按\(selection.title)计算续航与充电时间"
        }
        return "当前生效：\(chemistry.effectiveTitle) · \(chemistry.specificationText)"
    }

    private var nominalVoltage: Double? {
        Self.numberValue(nominalVoltageInput)
    }

    private var capacityWh: Double? {
        Self.numberValue(capacityWhInput)
    }

    private var hasValidSpecification: Bool {
        Self.isWithin(nominalVoltage, minimum: 12, maximum: 200, allowingEmpty: nominalVoltageInput)
            && Self.isWithin(capacityWh, minimum: 100, maximum: 20_000, allowingEmpty: capacityWhInput)
    }
    ...
    private static func numberValue(_ input: String) -> Double? {
        let trimmed = input.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? nil : Double(trimmed)
    }

    private static func isWithin(_ value: Double?, minimum: Double, maximum: Double, allowingEmpty input: String) -> Bool {
...
private struct BatterySpecificationInput: View {
    var title: String
...
private struct BatteryDetailHeroCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack(spacing: 10) {
                ...
                if isLithiumBattery {
                    BatteryDetailMiniMetric(title: "循环", value: snapshot.state.batteryCycleCountText, systemImage: "arrow.trianglehead.2.clockwise")
                }
            }
        }
        .padding(18)
    ...
    private var isLithiumBattery: Bool {
        snapshot.state.serverPrediction?.batteryChemistry?.effective == NinebotBatteryChemistry.lithium.rawValue
    }
}

private struct BatteryHistoryTrendCard: View {
    var points: [NinebotVehicleHistoryPoint]

    private let chartHeight: CGFloat = 164

    private var samples: [NinebotVehicleHistoryPoint] {
        Array(
            points
                .filter { $0.battery != nil || $0.totalMileage != nil }
                .sorted { $0.date < $1.date }
                .suffix(48)
        )
    }

    private var summary: NinebotVehicleHistorySummary? {
        NinebotVehicleHistorySummary(points: samples)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .top) {
                ...
                HStack(spacing: 10) {
                    TrendLegend(color: Color.teslaGreen, title: "电量", value: summary?.batteryDeltaText ?? "--%")
                    TrendLegend(color: Color.teslaPrimaryText, title: "行程", value: summary?.mileageDeltaText ?? "-- km")
                }
            }

            if samples.count >= 2 {
                BatteryHistoryLineChart(samples: samples, height: chartHeight)
            } else {
                ContentUnavailableView {
                    Label("等待趋势数据", systemImage: "chart.xyaxis.line")
                } description: {
                    Text("刷新车况后会记录电量与行程变化")
                }
...
private struct BatteryHistoryLineChart: View {
    var samples: [NinebotVehicleHistoryPoint]
    var height: CGFloat
        ...
        VStack(spacing: 7) {
            ...
            GeometryReader { proxy in
                Canvas { context, size in
                    ...
                    drawLine(
                        values: samples.map { $0.battery.map(Double.init) },
                        range: 0...100,
                    ...
                    drawLine(
                        values: samples.map(\.totalMileage),
                        range: mileageRange,
                        color: Color.teslaPrimaryText,
                        in: &context,
                        size: size
                    )
                }
            ...
            HStack {
                Text(formatTrendTime(samples.first?.date))
                Spacer()
                Text("0%")
                Spacer()
                Text(formatTrendTime(samples.last?.date))
            }
            .font(.caption2.monospacedDigit())
            .foregroundStyle(Color.teslaSecondaryText)
        }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("电量与行程趋势图")
    }

    private var mileageRange: ClosedRange<Double> {
        let values = samples.compactMap(\.totalMileage)
        guard let minimum = values.min(), let maximum = values.max() else { return 0...1 }
        if maximum - minimum < 0.01 { return minimum...(minimum + 1) }
        return minimum...maximum
    }

    private var totalMileageTopLabel: String {
        guard let value = samples.compactMap(\.totalMileage).max() else { return "-- km" }
        return "\(formatTrendMileage(value)) km"
    }

    private func drawGrid(in context: inout GraphicsContext, size: CGSize) {
        let gridColor = Color.teslaHairline.opacity(0.8)
        for fraction in [0.25, 0.5, 0.75] {
            let y = size.height * fraction
            var path = Path()
            path.move(to: CGPoint(x: 0, y: y))
            path.addLine(to: CGPoint(x: size.width, y: y))
            context.stroke(path, with: .color(gridColor), style: StrokeStyle(lineWidth: 1, dash: [3, 4]))
        }
    }
    ...
    private func drawLine(
        ...
        guard values.compactMap({ $0 }).count >= 2 else { return }
        let lower = range.lowerBound
        let span = max(range.upperBound - lower, 0.0001)
        ...
        for (index, value) in values.enumerated() {
            ...
            let x = inset + usableWidth * CGFloat(index) / CGFloat(values.count - 1)
            let progress = min(max((value - lower) / span, 0), 1)
            let y = inset + usableHeight * (1 - CGFloat(progress))
            if !hasActiveSegment {
    ...
    private func formatTrendTime(_ date: Date?) -> String {
        guard let date else { return "--:--" }
        return Self.timeFormatter.string(from: date)
    }

    private func formatTrendMileage(_ value: Double) -> String {
        Self.mileageFormatter.string(from: NSNumber(value: value)) ?? "--"
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            DetailSection(title: "充电信息") {
                ...
                DetailRow(title: snapshot.state.remainingChargeTimeSourceTitle, value: snapshot.state.remainingChargeTimeText, systemImage: "clock.badge.questionmark")
            }
        }
        .padding(16)
        .background(Color.teslaCardBackground)
...
private struct VehiclePickerRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            VehicleImage(vehicle: snapshot.vehicle, size: 52)
...
private struct VehicleControlHero: View {
    ...
    var isCapturePrivacyEnabled: Bool
    var isCapturePrivacyObscured: Bool
    var isLoading: Bool
    var onRingBell: () -> Void
    ...
    var body: some View {
        VStack(spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                        ...
                        if let coordinate = vehicleCoordinate(snapshot.state) {
                            NavigationLink {
                                ...
                                Text(resolvedAddress)
                                    ...
                                    .lineLimit(1)
                                    .truncationMode(.tail)
                                    .capturePrivacyProtected(
                                        isEnabled: isCapturePrivacyEnabled,
                                        isObscured: isCapturePrivacyObscured,
                                        cornerRadius: 6
                                    )
                ...
                VehicleHeroStatusStack(state: snapshot.state)
            }

            if snapshot.state.isPoweredOn == false {
                PowerOffWarningBanner()
            }
            ...
            ZStack(alignment: .bottom) {
                ...
                VehicleImage(vehicle: snapshot.vehicle, size: 246, showsBackground: false)
...
private struct AlgorithmEstimateTitle: View {
    var isUsingDefaultAlgorithm: Bool
    var font: Font
    var foreground: Color
    @State private var isShowingFallbackInfo = false
    ...
    var body: some View {
        HStack(spacing: 3) {
            ...
            if isUsingDefaultAlgorithm {
                Button {
                    isShowingFallbackInfo = true
                } label: {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel("查看算法预估说明")
            }
        }
        .alert("算法预估", isPresented: $isShowingFallbackInfo) {
            Button("知道了", role: .cancel) {}
        } message: {
            Text("由于样本量不足，为确保预估准确性已切换为默认算法")
        }
    }
}
...
private struct VehicleHeroStatusStack: View {
    var state: NinebotVehicleState

    var body: some View {
        StatusChip(
            title: lockTitle,
            systemImage: state.isLocked == true ? "lock.fill" : "lock.open.fill",
            color: state.isLocked == true ? Color.teslaSecondaryText : .orange
        )
    }

    private var lockTitle: String {
        guard let isLocked = state.isLocked else { return "锁车未知" }
        return isLocked ? "已上锁" : "已解锁"
    }
}

private struct PowerOffWarningBanner: View {
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: "exclamationmark.triangle.fill")
                .font(.headline.weight(.semibold))
        ...
        .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
        .shadow(color: Color.red.opacity(0.18), radius: 14, x: 0, y: 8)
    }
}
...
private struct VehicleHeroCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .top, spacing: 14) {
                VehicleImage(vehicle: snapshot.vehicle, size: 78)
            ...
            HStack(spacing: 22) {
                MetricView(title: snapshot.state.estimatedMileageSourceTitle, value: snapshot.state.officialEstimatedMileageText, systemImage: "road.lanes")
...
private struct BasicInfoTile: View {
    ...
    var value: String
    var systemImage: String
    var showsAlgorithmFallbackInfo = false
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            if showsAlgorithmFallbackInfo {
                AlgorithmEstimateTitle(
                    isUsingDefaultAlgorithm: true,
                    font: .caption2.weight(.medium),
                    foreground: Color.teslaSecondaryText
                )
            } else {
...
private struct VehicleDetailPanel: View {
    ...
    private var showsChargingTimeDetails: Bool {
        snapshot.state.isCharging == true && !snapshot.state.isFullyCharged
    }
...
private struct VehicleRow: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 12) {
                VehicleImage(vehicle: snapshot.vehicle, size: 48)
...
private struct EmptyDashboardView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text(hasConfiguration ? "暂无车辆数据" : "未配置服务器")
                .font(.headline)

            Text(hasConfiguration ? "刷新后会显示九号车辆状态" : "到“我的”填写服务器地址并绑定账号后即可读取车辆")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
        }
...
private struct VehicleImage: View {
    ...
    var vehicle: NinebotVehicleInfo? = nil
    var size: CGFloat
    ...
    private var effectiveURLString: String? {
        vehicle?.imageURLString(prefersDarkImage: colorScheme == .dark) ?? urlString
    }
