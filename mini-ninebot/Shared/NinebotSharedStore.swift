...
struct NinebotSharedStore {
    private enum Key {
        static let serverConfiguration = "ninebot.server.configuration"
        static let legacyConfiguration = "ninebot.proxy.configuration"
        ...
        static let capturePrivacyProtectionEnabled = "ninebot.capture.privacy.protection.enabled"
        static let pendingAppRoute = "ninebot.pending.app.route"
        static let chargingLiveActivityPushToStartToken = "ninebot.charging.live.activity.push.to.start.token"
        static let chargingLiveActivityPushTokenPrefix = "ninebot.charging.live.activity.push.token."
        static let chargingLiveActivityPushTokenRecords = "ninebot.charging.live.activity.push.token.records"
    }
    ...
    func loadConfiguration() -> NinebotServerConfiguration? {
        if let data = defaults.data(forKey: Key.serverConfiguration),
           let configuration = try? decoder.decode(NinebotServerConfiguration.self, from: data) {
            return configuration
        }
        guard let data = defaults.data(forKey: Key.legacyConfiguration),
    ...
    func loadCapturePrivacyProtectionEnabled() -> Bool {
        defaults.bool(forKey: Key.capturePrivacyProtectionEnabled)
    }

    func saveCapturePrivacyProtectionEnabled(_ isEnabled: Bool) {
        defaults.set(isEnabled, forKey: Key.capturePrivacyProtectionEnabled)
    }

    func savePendingAppRoute(_ route: NinebotPendingAppRoute) {
        defaults.set(route.rawValue, forKey: Key.pendingAppRoute)
    }

    func consumePendingAppRoute() -> NinebotPendingAppRoute? {
        guard let rawValue = defaults.string(forKey: Key.pendingAppRoute),
              let route = NinebotPendingAppRoute(rawValue: rawValue) else {
            return nil
        }
        defaults.removeObject(forKey: Key.pendingAppRoute)
        return route
    }

    func loadChargingLiveActivityPushToStartToken() -> String? {
        let token = defaults.string(forKey: Key.chargingLiveActivityPushToStartToken)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return token.isEmpty ? nil : token
    }

    func saveChargingLiveActivityPushToStartToken(_ token: String) {
        defaults.set(token, forKey: Key.chargingLiveActivityPushToStartToken)
    }

    func loadChargingLiveActivityPushToken(activityID: String) -> String? {
        let token = defaults.string(forKey: chargingLiveActivityPushTokenKey(activityID: activityID))?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return token.isEmpty ? nil : token
    }

    func saveChargingLiveActivityPushToken(_ token: String, activityID: String, vehicleSN: String? = nil) {
        defaults.set(token, forKey: chargingLiveActivityPushTokenKey(activityID: activityID))
        var records = loadChargingLiveActivityPushTokenRecords()
        let cleanedSN = cleanedVehicleSN(vehicleSN) ?? records.first(where: { $0.activityID == activityID })?.vehicleSN
        ...
        records.removeAll { existing in
            ...
            if let cleanedSN, existing.vehicleSN == cleanedSN {
                defaults.removeObject(forKey: chargingLiveActivityPushTokenKey(activityID: existing.activityID))
                return true
            }
            return false
        }
        ...
        if cleanedSN == nil {
            records.removeAll { existing in
                let shouldReplace = existing.vehicleSN == nil
                if shouldReplace {
                    defaults.removeObject(forKey: chargingLiveActivityPushTokenKey(activityID: existing.activityID))
                }
        ...
        saveChargingLiveActivityPushTokenRecords(records)
    }

    func removeChargingLiveActivityPushToken(activityID: String) {
        defaults.removeObject(forKey: chargingLiveActivityPushTokenKey(activityID: activityID))
        let records = loadChargingLiveActivityPushTokenRecords().filter { $0.activityID != activityID }
        saveChargingLiveActivityPushTokenRecords(records)
    }

    func removeAllChargingLiveActivityPushTokens() {
        for record in loadChargingLiveActivityPushTokenRecords() {
            defaults.removeObject(forKey: chargingLiveActivityPushTokenKey(activityID: record.activityID))
        }
        defaults.removeObject(forKey: Key.chargingLiveActivityPushTokenRecords)
    }

    func pruneChargingLiveActivityPushTokens(activeActivityIDs: Set<String>) {
        let records = loadChargingLiveActivityPushTokenRecords().filter { record in
            let isActive = activeActivityIDs.contains(record.activityID)
            if !isActive {
                defaults.removeObject(forKey: chargingLiveActivityPushTokenKey(activityID: record.activityID))
            }
            return isActive
    ...
    func loadChargingLiveActivityPushTokenRecords() -> [NinebotLiveActivityPushTokenRecord] {
        guard let data = defaults.data(forKey: Key.chargingLiveActivityPushTokenRecords),
              let records = try? decoder.decode([NinebotLiveActivityPushTokenRecord].self, from: data) else {
            return []
        }
        return records
    }
    ...
    private func chargingLiveActivityPushTokenKey(activityID: String) -> String {
        "\(Key.chargingLiveActivityPushTokenPrefix)\(activityID)"
    }
    ...
    private func cleanedVehicleSN(_ value: String?) -> String? {
